...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted : Tweet = {
    if (left.isInstanceOf[Empty] && right.isInstanceOf[Empty]) elem
      ...
      var gtr = greater(left, right);
      if (elem.retweets > gtr.retweets) elem
      else gtr
    }
  }

  def descendingByRetweet : TweetList = {
    var most = this.mostRetweeted;
    var tail = this.remove(most);
    return new Cons(most, tail.descendingByRetweet)
  }

  def greater(x: TweetSet, y: TweetSet) : Tweet = {
    if (x.isInstanceOf[Empty]) y.mostRetweeted
    else if (y.isInstanceOf[Empty]) x.mostRetweeted
    else if (x.mostRetweeted.retweets > y.mostRetweeted.retweets) x.mostRetweeted
    else y.mostRetweeted
  }
